...
import { CreateWebSiteThemeDto, UpdateWebSiteThemeDto, WebSiteTheme } from '@/api/types/WebSite/useWebSiteTheme';
...
export function useWebSiteThemes() {
  ...
  const useGetActiveTheme = (websiteId: string, p0: { enabled: boolean; retry: number; onError: (err: any) => void; onSuccess: (data: any) => void; }) => {
    return useQuery({
